        ...
        >
          ...
          <div className="flex justify-center">
            {skillCategories.map((category, categoryIndex) => (
              ...
              >
                <Card
                  className={`glass-card ${getColorClass(category.color)} glow-on-hover`}
                >
                  <CardHeader>
                    <CardTitle
                      className={`text-lg sm:text-xl font-bold ${getColorClass(category.color)} flex items-center justify-center`}
                    >
                      <span
                        className={`w-3 h-3 bg-${category.color} rounded-full mr-3`}
                      ></span>
                      {category.title}
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4 lg:gap-6">
                      {category.skills.map((skill, skillIndex) => (
                        ...
                        >
                          <skill.icon
                            className={`text-2xl sm:text-3xl lg:text-4xl ${getColorClass(category.color)} group-hover:scale-110 transition-transform`}
                          />
            ...
            <Card className="glass-card border-primary/20 max-w-4xl mx-auto">
              <CardContent className="p-4 sm:p-6 lg:p-8">
                ...
                <div className="flex flex-wrap justify-center gap-2 sm:gap-3 lg:gap-4">
                  {["Next.js", "Express.js", "TypeScript"].map(
                    (skill, index) => (
                      <motion.span
                        key={skill}
